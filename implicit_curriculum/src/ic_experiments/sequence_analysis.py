...
def _safe_merge_task_meta(df: pd.DataFrame, meta: pd.DataFrame) -> pd.DataFrame:
    """Merge task metadata idempotently, avoiding pandas suffix collisions.

    Some analysis pipelines already merge metadata into acquisition/final tables.
    Re-merging can create duplicate columns such as `kind_meta`, which pandas now
    rejects. This helper drops stale metadata-suffix columns and only adds or fills
    canonical metadata columns.
    """
    if df.empty or meta.empty or "task_name" not in df.columns:
        return df.copy()
    out = df.drop(columns=[c for c in df.columns if c.endswith("_meta")], errors="ignore").copy()
    meta = meta.drop_duplicates("task_name").copy()
    for col in [c for c in meta.columns if c != "task_name" and c in out.columns]:
        # Keep existing values but fill missing values from metadata.
        fill = meta[["task_name", col]].rename(columns={col: f"__meta_{col}"})
        out = out.merge(fill, on="task_name", how="left")
        out[col] = out[col].where(out[col].notna(), out[f"__meta_{col}"])
        out = out.drop(columns=[f"__meta_{col}"])
    add_cols = [c for c in meta.columns if c == "task_name" or c not in out.columns]
    if len(add_cols) > 1:
        out = out.merge(meta[add_cols], on="task_name", how="left")
    return out
...
def stratified_ordering_summary(
    ...
    if acq.empty:
        return pd.DataFrame()
    acq = _safe_merge_task_meta(acq, meta)
    final = final_df.copy()
    if "frequency" not in final.columns:
        final = _safe_merge_task_meta(final, meta)
